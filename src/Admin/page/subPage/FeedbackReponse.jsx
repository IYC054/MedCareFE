...
const FeedbackReponse = () => {
    ...
    useEffect(() => {
        const fetchNews = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/api/sendmail/${id}`, {
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg">
            <h2 className="text-xl font-semibold mb-4">Feedback Của {feedback.fullname}</h2>
            <div className="mb-4">

                <p><strong>Email:</strong> {feedback.sender_email}</p>
                <p><strong>Thời gian:</strong>  {new Date(feedback.createdAt).toLocaleString()}</p>
                <p><strong>Số liên lạc:</strong> {feedback.phone}</p>
...
export default FeedbackReponse;

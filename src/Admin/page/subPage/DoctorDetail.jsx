...
<<<<<<< HEAD
    ...
    const handleEditSchedule = async () => {
        ...
        try {
            // Gửi yêu cầu DELETE để xóa lịch cũ trước khi thêm lịch mới
            ...
            // Sau khi xóa thành công, gửi yêu cầu POST để thêm lịch mới
    ...
    const fetchDoctorData = async () => {
        try {
            ...
            const [doctorResponse, patientfileResponse, workingHoursResponse] = await Promise.all([
...
>>>>>>> 1d541e29c3f70fa5e75ddc5222dca8a915ccbe15
            ...
            if (doctorData) {
                ...
                const patientsForDoctor = patientfileResponse.data.filter(pa => pa.doctor_id === parseInt(id));
                setPatientFile(patientsForDoctor);

                // Lọc lịch làm việc của bác sĩ
                const doctorWorkingHours = workingHoursResponse.data.filter(item => item.doctor_id === parseInt(id));
...
<<<<<<< HEAD
    ...
    const handleShiftUpdate = async () => {
        try {
            ...
            await axios.put(`http://localhost:8080/api/workinghours/${selectedShiftId}`, {
                startTime: selectedShift === 'morning' ? "08:00:00" : "13:00:00",
                endTime: selectedShift === 'morning' ? "12:00:00" : "16:00:00",
                workDate: selectedWorkDate,
            }, {
            ...
            setWorkingHours(prevState => prevState.map(item => {
    ...
    return (
        <div className="bg-gray-100 min-h-screen p-6 relative" id="goup" >
            <div className="container mx-auto bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="mt-6">
                    <h2 className="text-xl font-semibold text-gray-700 mb-4">Lịch làm việc của bác sĩ</h2>
                    <div className="overflow-x-auto max-h-96 overflow-y-auto">
                        <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                            <thead className="sticky top-0 z-10">
                                <tr className="bg-gray-50">
                                    <th className="text-left px-4 py-2 text-sm font-medium text-gray-500">Ngày</th>
                                    <th className="text-left px-4 py-2 text-sm font-medium text-gray-500">Ca làm việc</th>
                                    <th className="text-left px-4 py-2 text-sm font-medium text-gray-500">Giờ bắt đầu</th>
                                    <th className="text-left px-4 py-2 text-sm font-medium text-gray-500">Giờ kết thúc</th>
                                    <th className="text-left px-4 py-2 text-sm font-medium text-gray-500">Cập nhật</th>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        <tr key={index} className="bg-gray-50 border-b">
                                            ...
                                            <td className="px-4 py-2 text-gray-700">
                                                <button
                                                    ...
                                                    onClick={() => openUpdateModal(shift.startTime === "08:00:00" ? 'morning' : 'afternoon', shift.id, shift.workDate)}
                                                >
                                                    Cập nhật
                                                </button>
                                            </td>
                        ...
                        {isModalOpen && (
                            ...
                            >
                                ...
                                >
                                    <h2 className="text-2xl font-bold mb-4 text-center">Chỉnh sửa ca làm việc của ngày </h2>
                                    <div className="mb-6">
                                        <label className="block text-gray-700 font-medium mb-2">Chọn ca làm việc:</label>

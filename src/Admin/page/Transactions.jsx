...
function Transactions() {
  ...
  const [vipappointmentsData, setFilteredVipAppointments] = useState([]);
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        const appointmentIdsFromPayments = paymentsData.map(
          (payment) => payment.appointment_id
        );
        ...
        const responsevipAppointments = await axios.get(
          "http://localhost:8080/api/vip-appointments"
        );
        const vipappointmentsData = responsevipAppointments.data;
        setFilteredVipAppointments(vipappointmentsData);
        ...
        const patientAccountIds = matchingAppointments.map(
          (appointment) => appointment.patient.account_id
        );
        ...
        const totalRevenue = limitedTransactions.reduce(
          (sum, item) =>
            item.status === "Đã thanh toán" ? sum + item.amount : sum,
          0
        );
        ...
        const totalfail = limitedTransactions.reduce(
          (sum, item) =>
            item.status === "Chưa thanh toán" ? sum + item.amount : sum,
          0
        );
        ...
        const totalpending = limitedTransactions.reduce(
          (sum, item) =>
            item.status === "Chờ xử lý" ? sum + item.amount : sum,
          0
        );
        ...
        const totalcashback = limitedTransactions.reduce(
          (sum, item) =>
            item.status === "Hoàn tiền" ? sum + item.amount : sum,
          0
        );
  ...
  const handleDetailClick = (id, vip) => {
    navigate(`/admin/transaction/detail/${id}?isvip=${vip}`);
  };
  const handleExportExcel = () => {
    ...
    const exportData = transactions.map((tra, index) => ({
      ...
      "Số điện thoại": appointmentsData.doctor.vip
        ? vipappointmentsData.find((a) => a.id === tra.vipappointment_id)
            ?.patient?.account?.phone || "N/A"
        : appointmentsData.find((a) => a.id === tra.appointment_id)?.patient
            ?.account?.phone || "N/A",
      "Tên khách hàng": appointmentsData.doctor.vip
        ? vipappointmentsData.find((a) => a.id === tra.vipappointment_id)
            ?.patient?.account?.name || "N/A"
        : appointmentsData.find((a) => a.id === tra.appointment_id)?.patient
            ?.account?.name || "N/A",
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className=" flex justify-between items-center bg-[#e5f6fd] py-2 px-4 rounded-md mb-2">
        <div className="font-bold text-black">
          50 GIAO DỊCH MỚI NHẤT TRONG NGÀY
        </div>
        <button
        ...
        >
          <i
            className={`bi ${showForm ? "bi-x-circle-fill" : "bi-funnel-fill"}`}
          ></i>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
          <div className="text-sm font-medium text-gray-900 border-1 bg-[#fff9e2] rounded-lg p-3">
            <i className="bi bi-arrow-counterclockwise text-orange-700 px-3 text-lg"></i>
            GIAO DỊCH HOÀN TIỀN
          </div>
          <div className="flex align-center justify-between">
            <div className="text-md text-gray-600 pl-4 pr-4 text-center py-3">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
          <div className="text-sm font-medium text-gray-900 border-1 bg-[#ffefdd] rounded-lg p-3">
            <i className="bi bi-clock-history text-yellow-700 px-3 text-lg"></i>
        ...
        <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
          <div className="text-sm font-medium text-gray-900 border-1 bg-[#ffebe9] rounded-lg p-3">
            <i className="bi bi-exclamation-circle px-3 text-red-700 text-lg"></i>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200">
        <div className="p-4 flex justify-between items-center border-b border-gray-200">
          <h2 className="text-lg font-semibold text-[#da624a]">
            Danh sách giao dịch
          </h2>
          <button
            ...
            <tbody>
              {transactions.length > 0 ? (
                transactions.map((t, index) => {
                  ...
                  const dataSource = t.vipappointment_id
                    ? vipappointmentsData
                    : appointmentsData;

                  // Tìm kiếm transactionAppointment trong danh sách phù hợp
                  const transactionAppointment = dataSource.find(
                    (appointment) =>
                      appointment.id ===
                      (t.vipappointment_id || t.appointment_id)
                  );
                  console.log("ASDASDAS" + JSON.stringify(t));
                  return (
                    <tr key={t.id} className="border-b">
                      ...
                      <td className="p-4 text-sm">
                        {transactionAppointment
                          ? transactionAppointment.patientprofile.phone
                          : "N/A"}
                      </td>
                      <td className="p-4 text-sm">
                        {transactionAppointment
                          ? transactionAppointment.patientprofile.fullname
                          : "N/A"}
                      </td>
                      ...
                      <td className="p-4">
                        <button
                          className="text-sm text-[#da624a] hover:underline"
                          onClick={() => handleDetailClick(t.id, t.vipappointment_id
                            ? true
                            : false)}
                        >
                          Chi tiết

...
const Login = ({ email, close }) => {
    ...
    if (email) {
      try {
        const response = await fetch(
          `http://localhost:8080/api/auth/forgot-password?email=${encodeURIComponent(
            email
          )}`,
          {
            method: "POST",
  ...
  if (loading) {
    return (
      <div className="flex-1 p-8 items-center justify-center flex flex-col mt-40">
    ...
    <div className="flex-1">
      {!next && (
        <div className="flex-1 p-8">
          ...
          <span
            className="text-[rgb(117,145,244)] text-sm cursor-pointer"
